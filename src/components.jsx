    ...
    <div onClick={handleClick} style={{ background: "white", borderRadius: 16, marginBottom: 12, cursor: "pointer", boxShadow: "0 12px 32px rgba(0,0,0,0.08)", border: isExpired ? "1px solid #D1D5DB" : "1px solid #EFEFEF", overflow: "hidden", transition: "transform 0.12s ease, box-shadow 0.12s ease", opacity: isExpired ? 0.6 : 1, filter: isExpired ? "grayscale(0.7)" : "none", position: "relative" }}>
        ...
        >
          ...
          {hasLogo && (
            <div style={{ position: "absolute", bottom: 10, left: 10, background: "white", borderRadius: 10, padding: "3px 8px 3px 4px", display: "flex", alignItems: "center", gap: 5, boxShadow: "0 1px 4px rgba(0,0,0,0.15)" }}>
              ...
              <span style={{ fontSize: 17, fontWeight: 1000, color: "#111827" }}>{item.name}{qualifiedForBadge && <img src="/verified-badge.svg" width={17} height={17} style={{ marginLeft:5, verticalAlign:"middle", display:"inline-block", filter: item.is_local_favourite ? "sepia(1) saturate(4) hue-rotate(5deg) brightness(1.1)" : "none" }} alt="Verified" />}</span>
            </div>
          )}
          {/* Fav button */}
      ...
      <div style={{ padding: "10px 14px 13px", marginTop: 2 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 8, marginBottom: 5 }}>
          <span style={{ fontSize: 17, fontWeight: 700, color: "#111827", lineHeight: 1.3 }}>{item.name}{qualifiedForBadge && <img src="/verified-badge.svg" width={17} height={17} style={{ marginLeft:5, verticalAlign:"middle", display:"inline-block", filter: item.is_local_favourite ? "sepia(1) saturate(4) hue-rotate(5deg) brightness(1.1)" : "none" }} alt="Verified" />}</span>
...
export function DetailView({ item, onBack, userLoc, reviews, onAddReview, isFav, onToggleFav, onAddToCalendar, onRemoveFromCalendar, calendarPlan, isVisited, onToggleVisited, tips = [], onAddTip, allListings = [], onSelectListing, crossLinks = [] }) {
  const tc = typeColors[item.type] || { bg: "#eee", color: "#333" };
  ...
  return (
    <div style={{ flex: 1, overflowY: "auto" }}>
      ...
      <div style={{ padding: 20 }}>
        ...
        <div style={{ fontSize: 18, color: tc.color, fontWeight: 800, marginBottom: 8, display: "flex", alignItems: "center", gap: 4 }}>
        ...
        {item.cta && item.cta.url && item.website && item.website !== item.cta.url && (
          <div style={{ marginTop: 8 }}>
            <button onClick={(e) => { e.stopPropagation(); openExternalWebsite(item.website); }} style={{ width: "100%", padding: 11, borderRadius: 12, border: "1px solid #E5E7EB", background: "white", color: "#374151", fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: "inherit" }}>
              🔗 More info / Bookings
            </button>
          </div>
        )}
        ...
        {item.whatsappGroup && (
          <div style={{ marginTop: 8 }}>
            <button onClick={(e) => { e.stopPropagation(); window.open(item.whatsappGroup, "_blank"); }} style={{ width: "100%", padding: 11, borderRadius: 12, border: "1px solid #dcfce7", background: "#f0fdf4", color: "#166534", fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              💬 Join WhatsApp Group
            </button>
          </div>
        )}
        {item.instagram && (
          <div style={{ marginTop: 8 }}>
            <button onClick={(e) => { e.stopPropagation(); window.open("https://www.instagram.com/" + item.instagram, "_blank"); }} style={{ width: "100%", padding: 11, borderRadius: 12, border: "1px solid #fce7f3", background: "#fdf2f8", color: "#9d174d", fontSize: 15, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
              📸 View on Instagram
            </button>
          </div>
        )}
